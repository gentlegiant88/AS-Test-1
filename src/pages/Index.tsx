...
import { useToast } from "@/hooks/use-toast";const DOMAIN_NAME = "lasvegascybertruck.com";
const AUCTION_END_DATE = new Date("2026-06-15T00:00:00-07:00"); // Midnight Pacific Time (end of June 14th)interface Bid {
  id: string;
  amount: number;
  bidder: string;
  name?: string;
  ...
  maxAmount?: number;
  timestamp: Date;
}const INITIAL_BIDS: Bid[] = [];const Index = () => {
  ...
  const [pin, setPin] = useState<string>("");  const [currentUser, setCurrentUser] = useState<{name: string, email: string, pin?: string} | null>(() => {
  ...
  }, [currentUser]);  const [isLoginOpen, setIsLoginOpen] = useState<boolean>(false);
  const [loginEmail, setLoginEmail] = useState<string>("");
  const [loginPin, setLoginPin] = useState<string>("");  const hasBid = currentUser ? bids.some(b => b.email?.toLowerCase() === currentUser.email.toLowerCase()) : false;
  const [timeLeft, setTimeLeft] = useState<{ days: number; hours: number; minutes: number; seconds: number }>({ days: 0, hours: 0, minutes: 0, seconds: 0 });
  const [isAuctionEnded, setIsAuctionEnded] = useState<boolean>(false);
  const [isEditingMaxBid, setIsEditingMaxBid] = useState<boolean>(false);
  const [editMaxBidAmount, setEditMaxBidAmount] = useState<string>("");  const highestBid = bids.length > 0 ? Math.max(...bids.map(b => b.amount)) : 0;
  const minNextBid = highestBid + 500;  useEffect(() => {
    const calculateTimeLeft = () => {
      const now = new Date().getTime();
      const distance = AUCTION_END_DATE.getTime() - now;  if (distance <= 0) {
...
return () => clearInterval(timer);  }, []);  const handleBid = (e: React.FormEvent) => {
    e.preventDefault();
    if (isAuctionEnded) {
    ...
    const amount = parseFloat(bidAmount.replace(/,/g, ''));if (isNaN(amount)) {
...
toast({ 
  ...
  className: "bg-primary text-primary-foreground border-none"
    ...
    e.preventDefault();
    const userBids = bids.filter(b => b.email?.toLowerCase() === loginEmail.toLowerCase());if (userBids.length > 0) {
